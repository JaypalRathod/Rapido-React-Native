...
const Selectlocations = () => {
  ...
  const checkDistance = async () => {
    ...
    if (lat1 === lat2 && lon1 === lon2) {
      Alert.alert("Pickup and drop locations can not be same. please select a different location")
      return
    }
    ...
    if (distance < minDistance) {
      Alert.alert("Selected Location are too close, please select location again..")
    } else if (distance > maxDistance) {
      Alert.alert("Selected Location are too far apart, please select location again..")
    } else {
      setLocations([])
      router.navigate({
    ...
    <View style={homeStyles.container}>
      ...
      <View style={uiStyles.locationInputs}>
        <LocationInput
          ...
          type='pickup'
          value={pickup}
          onChangeText={(text) => { setPickup(text), fetchLocation(text) }}
        ...
        <LocationInput
          ...
          type='drop'
          value={drop}
          onChangeText={(text) => { setDrop(text), fetchLocation(text) }}
      ...
      {isMapModalVisible &&
        <MapPickerModal
          selectedLocation={{
            ...
            longitude: focusedInput === 'drop' ? dropCoords?.logitude : pickupCoords?.logitude,
